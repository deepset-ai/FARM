...
from farm.conversion.converter import Converter
...
def test_conversion_adaptive_model_qa():
    ...
    transformer_model = farm_model.convert_to_transformers()
...
def test_conversion_adaptive_model_classification():
    ...
    transformer_model = farm_model.convert_to_transformers()
    transformer_model2 = AutoModelForSequenceClassification.from_pretrained("ddeepset/bert-base-german-cased-hatespeech-GermEval18Coarse")
    # compare weights
    for p1, p2 in zip(transformer_model.parameters(), transformer_model2.parameters()):
        assert (p1.data.ne(p2.data).sum() == 0)
...
def test_conversion_inferencer_ner():
    ...
    # save it
    farm_model_dir = "testsave/bert-conversion-test-hf"
    nlp.save(farm_model_dir)

    # load from disk in FARM format
    model = Converter.convert_from_transformers(farm_model_dir, device="cpu")
    tokenizer = Tokenizer.load(farm_model_dir)
    ...
    result_transformers = nlp(text)
    # Transformers model contains wrong labels and wrong number of labels
