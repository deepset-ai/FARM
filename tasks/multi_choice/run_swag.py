...
import numpy as np
import torch
from farm.modeling.bert.modeling import BertForMultipleChoice
...
def main():
    ...
    if args.do_train:
        train_examples = read_swag_examples(
            os.path.join(args.data_dir, "train.csv"), is_training=True
        )
    ...
    model = BertForMultipleChoice.from_pretrained(
        ...
        cache_dir=os.path.join(
            str(FARM_CACHE), "distributed_{}".format(args.local_rank)
        ),
        num_choices=4,
    )
    if args.fp16:
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        eval_examples = read_swag_examples(
            os.path.join(args.data_dir, "val.csv"), is_training=True
        )
