...
def convert_example_to_features(example, tokenizer, max_seq_length):
    ...
    assert (
        len(tokens) == len(segment_ids) <= max_seq_length
...
class PregeneratedDataset(Dataset):
    def __init__(
        ...
        if reduce_memory:
            ...
            input_ids = np.memmap(
                filename=self.working_dir / "input_ids.memmap",
                mode="w+",
                dtype=np.int32,
            ...
            segment_ids = np.memmap(
                filename=self.working_dir / "segment_ids.memmap",
                shape=(num_samples, seq_len),
                mode="w+",
                dtype=np.bool,
            )
            lm_label_ids = np.memmap(
                filename=self.working_dir / "lm_label_ids.memmap",
                shape=(num_samples, seq_len),
                mode="w+",
                dtype=np.int32,
            )
            lm_label_ids[:] = -1
            is_nexts = np.memmap(
                filename=self.working_dir / "is_nexts.memmap",
                shape=(num_samples,),
                mode="w+",
                dtype=np.bool,
            )
