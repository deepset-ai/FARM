...
def semantic_answer_similarity(result, sts_model_path_or_string="paraphrase-xlm-r-multilingual-v1", debug=False):
    ...
    :return best_pred_similarity, all_preds_highest_similarity, result, Optional[List[Dict]]
    """
    ...
    # flatten predictions and labels into one list to batch the encoding
    lengths = []
    all_texts = []
    ...
    if debug:
        ...
        return np.mean(top1_sim), np.mean(topn_sim), result
