...
from farm.modeling.prediction_head import QuestionAnsweringHead, TextClassificationHead, TokenClassificationHead, \
    RegressionHead, BertLMHead
...
class Converter:
    ...
    def convert_to_transformers(adaptive_model):
        """
        Convert an adaptive model to huggingface's transformers format. Produces one model for each prediction head.
        ...
        if task_type is None:
            ...
            if "MaskedLM" in architecture:
                ...
                logger.error("Could not infer task type from model config. Please provide task type manually. "
                             "('lm', 'question_answering', 'regression', 'text_classification' or 'ner')")

        if task_type == "lm":
            ph = BertLMHead.load(model_name_or_path)
            adaptive_model = AdaptiveModel(language_model=lm, prediction_heads=[ph], embeds_dropout_prob=0.1,
...
if __name__ == "__main__":
    converter = Converter()
    lm = LanguageModel.load("oliverguhr/german-sentiment-bert")
    ph = TextClassificationHead.load("oliverguhr/german-sentiment-bert")
    adaptive_model = AdaptiveModel(language_model=lm, prediction_heads=[ph], embeds_dropout_prob=0.1,
                                   lm_output_types="per_sequence", device="cpu")
    converter.convert_to_transformers(adaptive_model)
